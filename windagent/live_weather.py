...
def _source_hash(payload: dict[str, Any]) -> str:
    # API server processing time is not weather; refresh must not look like a
    # new meteorological input solely because generationtime_ms changed.
    content = {key: value for key, value in payload.items() if key != "generationtime_ms"}
...
def _finite_number(value: Any, label: str) -> float:
    if isinstance(value, bool):
        raise LiveWeatherUnavailableError(f"invalid boolean {label}")
    try:
        number = float(value)
...
def _validate_payload(payload: dict[str, Any], *, turbine_id: int, horizon: int,
    ...
    if not frame["wind_speed"].between(0, 100).all() or not frame["temperature"].between(-90, 70).all():
        raise LiveWeatherUnavailableError("forecast contains negative or implausible wind/temperature")
    ...
    if any(not 0 <= current_values[k] <= 100 for k in ("wind_speed_10m", "wind_speed_100m")) or not -90 <= current_values["temperature_2m"] <= 70:
        raise LiveWeatherUnavailableError("current response contains negative or implausible wind/temperature")
    reference_now = now if now is not None else pd.Timestamp.now(tz="UTC")
    if valid_time > reference_now + pd.Timedelta(minutes=15) or valid_time < reference_now - pd.Timedelta(hours=1):
...
def _load_cache(path: Path, *, turbine_id: int, horizon: int, start: pd.Timestamp,
    ...
    try:
        ...
        if (record.get("schema_version") != CACHE_SCHEMA or record.get("source") != SOURCE
                or record.get("endpoint") != API_URL or record.get("request_params") != _params(turbine_id, horizon, start)
                or record.get("turbine_id") != turbine_id or record.get("horizon") != horizon or record.get("forecast_start") != start.isoformat()):
            return None
...
def fetch_live_weather(turbine_id: int, horizon: int, cache_dir: Path, refresh: bool = False,
    ...
    if isinstance(horizon, bool) or not isinstance(horizon, int) or horizon not in (24, 48):
    ...
    if start != now_utc.floor("h") + pd.Timedelta(hours=1):
        raise ValueError("forecast_start must be the next full UTC hour")
    ...
    if not refresh:
        ...
        if cached is not None:
            ...
            return {"frame": frame, "current": current, "provenance": {
                ...
                "availability_basis": "actual_retrieval", "current_is_model_estimate": True,
    ...
    retrieved = _clock_now()
    # Validate age against completed retrieval, not only the pre-request clock.
    frame, current, returned = _validate_payload(payload, turbine_id=turbine_id, horizon=horizon, start=start, now=retrieved)
    retrieved_at = retrieved.isoformat()
    source_hash = _source_hash(payload)
    record = {"schema_version": CACHE_SCHEMA, "source": SOURCE, "endpoint": API_URL,
              "request_params": params, "turbine_id": turbine_id, "horizon": horizon,
